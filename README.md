...
比赛 H5 解谜游戏项目（≥50 关、纯玩法）。比赛不要求剧情与美术包装，本项目只做**零美术素材的轻包装**（方向见 [设计文档](docs/design.md) §10）。当前处于**双原型并行验证**阶段，同一套引擎骨架上跑两个候选玩法：

| 原型 | 代号 | 一句话规则 | 状态 |
| --- | --- | --- | --- |
| **《t+3》** | `src/games/t3` | 每次方向输入，会在延迟若干回合后再次控制另一枚棋子 | 教程批次 01–10 入库（v0 规则） |
| **《109.5°》** | `src/games/chem` | 只能从开口臂的背面撞入中心，撞入后中心构型翻转（SN2 立体化学的纯规则抽象） | 教程批次 01–10 入库（v0 规则） |
