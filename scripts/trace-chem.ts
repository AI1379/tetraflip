...
const fmt = (s: ChemState): string => {
  ...
  return [
    `P(${s.player}) 手持=${s.holding ?? '—'}${s.deprotected ? ' 已脱保护' : ''} 段${s.stage}/${s.stages.length}`,
    ...s.centers.map(
      (c, i) =>
        `#${i}${c.pos}${c.kind === 'trigonal' ? '(三)' : ''} 臂[${(
          ...
          .map((d) => `${d}${c.arms[d] ?? '·'}`)
          .join('|')}] 开口${c.leaving}${c.shielded && !s.deprotected ? ' 🛡' : ''}` +
        (activeGoals
          .filter((g) => g.center === i)
...
console.log(`${name} 最短解 ${result.solution.length} 步`)
